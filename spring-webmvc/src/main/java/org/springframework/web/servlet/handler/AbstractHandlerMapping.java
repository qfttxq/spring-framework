...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	protected HandlerInterceptor adaptInterceptor(Object interceptor) {
		//进行类型转换，拦截器有两种接口类型，分别是HandlerInterceptor 和 WebRequestInterceptor
		if (interceptor instanceof HandlerInterceptor) {
			return (HandlerInterceptor) interceptor;
		} else if (interceptor instanceof WebRequestInterceptor) {
			//WebRequestHandlerInterceptorAdapter的最上层接口为HandlerInterceptor（适配器模式）
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		// 遍历 adaptedInterceptors 列表，获得请求匹配的拦截器
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			//MappedInterceptor类型拦截器，需要匹配，若路径匹配，则添加到 chain 中
			if (interceptor instanceof MappedInterceptor) {
				...
				// 非MappedInterceptor类型拦截器，无需匹配，直接添加到 chain 中
